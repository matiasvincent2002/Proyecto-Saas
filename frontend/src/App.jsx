  ...
  async function handleCreateTask(event) {
    ...
    try {
      ...
      const task = await request('/api/v1/tasks', {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: JSON.stringify({ title: formData.get('title'), description: formData.get('description'), projectId: formData.get('projectId') || null, assigneeId: formData.get('assigneeId') || null, priority: formData.get('priority'), dueDate: formData.get('dueDate') || null }),
      })
